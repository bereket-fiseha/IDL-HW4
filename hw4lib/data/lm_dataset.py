...
class LMDataset(Dataset):
    ...
    def __init__(
        ...
        # Take subset
        self.length = int(len(self.text_files) * config.get('subset', 1.0))
        ...
        for file in tqdm(self.text_files):
            ...
            if isinstance(transcript, np.ndarray) and transcript.ndim == 0:
                transcript = transcript.item()
            if isinstance(transcript, (bytes, np.bytes_)):
