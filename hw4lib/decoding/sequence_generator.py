...
class SequenceGenerator:
    ...
    def generate_beam(
        ...
        # Initialize sequences (batch_size, beam_width, seq_len)
        sequences = x.unsqueeze(1).repeat(1, beam_width, 1)
        # Initialize scores (batch_size, beam_width)
        beam_scores = torch.full((batch_size, beam_width), -1e9, device=x.device)
        beam_scores[:, 0] = 0.0
        ...
        for _ in range(self.max_length - x.size(1)):
            ...
            for b in range(beam_width):
                # Only score if beam is not finished
                # But to keep batch operations, we score all and mask later or just score all.
                # score_fn takes (batch_size, seq_len)
                logits = self.score_fn(sequences[:, b, :]) # (batch_size, vocab_size)
                logits = self._apply_repeat_penalty(logits, sequences[:, b, :], repeat_penalty)
                logits = logits / temperature
                log_probs = torch.log_softmax(logits, dim=-1) # (batch_size, vocab_size)
                
                # If finished, the only possible next "token" is PAD with score 0 (logprob)
                # Or just keep the score as is.
                log_probs[finished[:, b], :] = -1e9
                log_probs[finished[:, b], self.tokenizer.pad_id] = 0.0
                
                all_next_scores.append(log_probs + beam_scores[:, b].unsqueeze(1))
            
            # stack to (batch_size, beam_width * vocab_size)
            all_next_scores = torch.stack(all_next_scores, dim=1).view(batch_size, -1)
            
            # Get top beam_width scores and indices
            top_scores, top_indices = torch.topk(all_next_scores, beam_width, dim=-1)
            
            # Convert indices back to beam_idx and token_id
            ...
            new_finished = []
            for b in range(beam_width):
                # Select the beams that were best
                prev_beams = sequences.gather(1, beam_indices[:, b].view(-1, 1, 1).expand(-1, 1, sequences.size(2))).squeeze(1)
                new_tokens = token_ids[:, b].unsqueeze(1)
                new_sequences.append(torch.cat([prev_beams, new_tokens], dim=1))
                
                # Update finished status
                prev_finished = finished.gather(1, beam_indices[:, b].unsqueeze(1)).squeeze(1)
                new_f = prev_finished | (token_ids[:, b] == self.tokenizer.eos_id)
                new_finished.append(new_f)
            
            sequences = torch.stack(new_sequences, dim=1)
            finished = torch.stack(new_finished, dim=1)
